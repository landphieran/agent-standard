...
6. In `extends` mode, grant the pipeline read access to the central template repository and consider an Azure Pipelines required-template check in addition to the immutable commit pin ([secure templates](https://learn.microsoft.com/en-us/azure/devops/pipelines/security/templates?view=azure-devops)).
7. Restrict branch-policy bypass, force-push, branch deletion, pipeline editing, pull-request status contribution, and template-repository write permissions to the least-privileged reviewed groups; keep break-glass membership explicit and time-bounded.
8. Run the remote audit below and retain its JSON output together with the administrator permission review before changing conformance state.
...
In an Azure Pipeline, the script can use `SYSTEM_COLLECTIONURI`, `SYSTEM_TEAMPROJECT`, and `BUILD_REPOSITORY_ID`. CI-enabled audits read `SYSTEM_ACCESSTOKEN` or `AZURE_DEVOPS_EXT_PAT` from the environment for the Advanced Security enablement query. Tokens are neither accepted on the command line nor written to output. The auditor performs GET/list operations only; it never applies policy.
...
An audit failure identifies stable `AS-ADO-*` control IDs. The script proves the declared branch-policy settings and Advanced Security enablement; it does not yet enumerate ACL/bypass membership or prove a required-template check. Its JSON output is therefore one evidence item, not an automatic conformance promotion. Administrative evidence and approval remain outside the repository until a future evidence-bundle format is standardized.
