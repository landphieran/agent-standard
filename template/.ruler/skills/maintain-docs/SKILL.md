...
Start with the governed documents declared in `.agent-standard/manifest.json`. Trace only the documents whose claims changed. Update code and docs together, refresh `last_verified` and `verified_against` only after checking the claim, and keep indexes and local links correct.
