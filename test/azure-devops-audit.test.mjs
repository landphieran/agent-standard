...
test('Azure DevOps remote evaluator ignores disabled duplicates before conformant policies', async t => {
  const { evaluateAzureDevOps } = await loadEvaluator(t)
  const withDisabledDuplicates = policies.flatMap(policy => [
    { ...structuredClone(policy), isEnabled: false, isBlocking: false },
    policy
  ])
  const result = evaluateAzureDevOps(config, withDisabledDuplicates, {
    codeSecurityFeatures: { codeSecurityEnabled: true },
    secretProtectionFeatures: { secretProtectionEnabled: true, blockPushes: true }
  })
